...
export const Login = () => {
  ...
  useEffect(() => {
    // Auto check relogin if user access /login
    relogin();
  }, []);

  const handleVisiblePw = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault();
  ...
  const handleLocalLogin = async (data: LocalLoginData) => {
    ...
    // Đăng nhập xong mới redirect (await login)
    await login.mutateAsync(loginRequest);
    ...
    (
      <div
        ...
        <>
          ...
          <Divider className="my-8 w-full bg-gray-600"/>

          {/* Login Form with React Hook Form */
          }
          <Form className="mx-auto gap-4 w-full" onSubmit={handleSubmit(handleLocalLogin)}>
            <Controller
      ...
      ${isSubmitting ? 'hover:bg-blue-500 hover:text-darkmode hover:border-white' : ''}`}
            >
              {login.isPending ? t('processing') : t('submit')}
            </Button>
          </Form>
          <div className="text-center mt-4 flex flex-col gap-2">
            ...
            <div>
              <Link href="/forgot-password" className="text-blue-400 inline">
                {t('forgotPassword')}
              </Link>
            </div>
          </div>
