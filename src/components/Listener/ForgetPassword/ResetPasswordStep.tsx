...
const ResetPasswordStep = ({email}: ResetPasswordStepProps) => {
  ...
  const conditions = {
    lowercase: {
      ...
      label: t("validation.passwordIncludeLowercase"),
    },
    uppercase: {
      met: /[A-Z]/.test(passwordValue || ""),
      label: t("validation.passwordIncludeUppercase"),
    },
    number: {
      met: /[0-9]/.test(passwordValue || ""),
      label: t("validation.passwordIncludeNumber"),
    },
    special: {
      met: /[@_\-=+.]/.test(passwordValue || ""),
      label: t("validation.passwordIncludeSpecial"),
    },
    length: {
      met: passwordValue?.length >= 8 && passwordValue?.length <= 30,
      label: t("validation.passwordLength"),
    },
  };
      ...
      <Form className="mx-auto gap-4 w-full" onSubmit={handleSubmit(handleResetPassword)}>
        ...
        <Input
          {...register("retypePassword", {
            required: t("validation.retypePasswordRequired"),
            validate: (value) => {
              if (value !== passwordValue) {
                return t("validation.retypePasswordMatch");
              }
            }
          })}
          name="retypePassword"
