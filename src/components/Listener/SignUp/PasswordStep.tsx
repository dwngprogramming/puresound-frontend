...
import ConditionalItem from "@/components/Listener/SignUp/ConditionalItem";
...
const PasswordStep = ({t, breakpoint}: PasswordStepProps) => {
  ...
  const conditions = {
    lowercase: {
      ...
      label: tValidation("passwordIncludeLowercase"),
    },
    uppercase: {
      met: /[A-Z]/.test(passwordValue || ""),
      label: tValidation("passwordIncludeUppercase"),
    },
    number: {
      met: /[0-9]/.test(passwordValue || ""),
      label: tValidation("passwordIncludeNumber"),
    },
    special: {
      met: /[@_\-=+.]/.test(passwordValue || ""),
      label: tValidation("passwordIncludeSpecial"),
    },
    length: {
      met: passwordValue?.length >= 8 && passwordValue?.length <= 30,
      label: tValidation("passwordLength"),
    },
  };
  ...
  const handleShowRetypePassword = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault();

    setShowPassword({
      ...showPassword,
      retypePassword: !showPassword.retypePassword
    });
  };
  ...
  return (
    <div
        ...
        render={({field}) => (
          <Input
            ...
            endContent={
              <button
                type="button"
                tabIndex={-1}   // Bỏ qua tab
                // Dùng onMouseDown thay cho onClick để tránh việc input bị mất focus -> onBlur của form được gọi.
      ...
      <div className="flex flex-col gap-2 mb-2 text-darkmode">
        <p className="text-sm text-gray-300 font-bold">{tValidation('passwordInclude')}</p>
          ...
          <Input
            ...
            endContent={
              <button
                type="button"
                tabIndex={-1}
                onMouseDown={handleShowRetypePassword}
                className="text-gray-400 hover:text-blue-400 transition-colors focus:cursor-pointer"
              >
                {
